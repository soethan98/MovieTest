...
class PopularDomainMapper  {

    fun mapItems(items: List<PopularMovieDataModel>): List<PopularDomainModel> {
        return items.map { mapItem(it) }

    }

    private fun mapItem(movie: PopularMovieDataModel): PopularDomainModel {
        with(movie) {
            return PopularDomainModel(
